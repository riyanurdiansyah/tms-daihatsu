...
const EditDialog: FC<IEditDialog> = ({
  ...
  const [dataOld, loadingDataOld] = useFetchUmum(`/api/partner/${id}`);
  ...
  useEffect(() => {
    if (dataOld) {
      ...
      setValue("video", dataOld?.data?.video);
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      const response = await axios.put(`${api_backend}/api/partner`, formData, {
        headers: {
